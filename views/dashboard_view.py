...
class Tooltip:
    ...
    def _on_motion(self, event=None):
        if self._tip_window:
            x = event.x_root + 14; y = event.y_root + 8
    ...
    def _show(self):
        ...
        label = ctk.CTkLabel(self._tip_window, text=self.text, font=ctk.CTkFont(size=11),
                             text_color="#f1f5f9", fg_color="#1e2140", padx=8, pady=4)
        label.pack()
        self._after_id = None
...
RANGE_OPTIONS = [
    ...
    ("2 Months", 60), ("3 Months", 90), ("6 Months", 180), ("Custom", None),
]
...
class DashboardView(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(head_row, text="Dashboard", font=ctk.CTkFont(size=28, weight="bold")).pack(side="left")

        cur_frame = ctk.CTkFrame(head_row, fg_color="transparent")
        cur_frame.pack(side="right")
        ctk.CTkLabel(cur_frame, text="Show in:", font=ctk.CTkFont(size=12),
                     text_color=COLORS["text_muted"]).pack(side="left", padx=(0, 6))
        self._currency_menu = ctk.CTkOptionMenu(cur_frame, variable=self._display_currency,
                                                 ...
                                                 dropdown_font=ctk.CTkFont(size=12), width=80,
        ...
        ctk.CTkLabel(self.scroll, text="Your financial snapshot", font=ctk.CTkFont(size=13),
                     text_color=COLORS["text_secondary"], anchor="w"
                     ).pack(anchor="w", padx=20, pady=(0, 6))

        range_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
        range_frame.pack(fill="x", padx=20, pady=(0, 6))
        ctk.CTkLabel(range_frame, text="Period:", font=ctk.CTkFont(size=12),
                     text_color=COLORS["text_muted"]).pack(side="left", padx=(0, 8))

        self._range_btns = []
        for label, days in RANGE_OPTIONS:
            btn = ctk.CTkButton(range_frame, text=label, width=72, height=28, corner_radius=8,
                                ...
                                hover_color="#1a1f3a", command=lambda d=days: self._set_range(d))
        ...
        ctk.CTkLabel(self._custom_frame, text="From:", font=ctk.CTkFont(size=12),
                     text_color=COLORS["text_muted"]).pack(side="left")
        self._start_label = ctk.CTkLabel(self._custom_frame,
                                          text=self._custom_start.strftime("%d %b %Y"),
        ...
        ctk.CTkLabel(self._custom_frame, text="To:", font=ctk.CTkFont(size=12),
                     text_color=COLORS["text_muted"]).pack(side="left")
        self._end_label = ctk.CTkLabel(self._custom_frame,
                                        text=self._custom_end.strftime("%d %b %Y"),
                                        font=ctk.CTkFont(size=12, weight="bold"),
                                        text_color=COLORS["text_primary"], width=90)
    ...
    def _show_undo_delete(self):
        try:
            deleted = self.api.get_deleted_transactions()
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load deleted transactions: {e}",
                                 parent=self.winfo_toplevel())
            return
        if not deleted:
            messagebox.showinfo("Undo Delete", "No recently deleted transactions.",
                                parent=self.winfo_toplevel())
            return
        ...
        for tx in deleted:
            ...
            ctk.CTkLabel(row, text=f"{tx_date.strftime('%d %b')}  {tx['category_name']}  \u20B9{tx['amount']:,.2f}",
    ...
    def _restore_tx(self, tx, popup):
        try:
            self.api.restore_transaction(tx["id"])
            self._refresh_data()
            popup.destroy()
        except Exception as e:
    ...
    def _refresh_data(self):
        ...
        try:
            result = self.api.get_transactions(limit=5000)
            txs_raw = result.get("transactions", [])
            range_start, range_end = self._get_date_range()
            self._all_txs = []
            for t in txs_raw:
                tx_date = datetime.strptime(t["transaction_date"], "%Y-%m-%d").date()
            ...
            if q:
                self._all_txs = [t for t in self._all_txs
                                 ...
                                 or q in t.get("currency", "").lower()
                                 or q in t["transaction_date"]
                                 or q in f"{t['amount']:.2f}"]

            income_total = sum(convert_amount(t["amount"], t.get("currency", "INR"), cur)
                               for t in self._all_txs if t["type"] == "income")
            expense_total = sum(convert_amount(t["amount"], t.get("currency", "INR"), cur)
                                for t in self._all_txs if t["type"] == "expense")
            ...
            self._all_categories = self.api.get_categories("expense")
            self._all_budgets = self.api.get_budgets()
    ...
    def _draw_pie_chart(self, cur):
        ...
        for t in expenses:
            cat_totals[t["category_name"]] += convert_amount(t["amount"], t.get("currency", "INR"), cur)
        color_map = {c["name"]: c["color"] for c in self._all_categories}
        labels = list(cat_totals.keys())
        sizes = list(cat_totals.values())
        colors_pie = [color_map.get(l, "#6366f1") for l in labels]

        budget_map = {b["category_name"]: b["monthly_limit"] for b in self._all_budgets}
        ...
        wedges, texts, autotexts = ax.pie(sizes, labels=None, autopct="", startangle=140,
                                           ...
                                           wedgeprops={"linewidth": 1.5, "edgecolor": "#13172b"})
        sym = PLAIN.get(cur, cur)
        legend_labels = []
        for l, s in zip(labels, sizes):
            warn = " \u26A0" if l in over_budget_cats else ""
            legend_labels.append(f"{l}{warn} ({sym}{s:,.0f})")
        ax.legend(wedges, legend_labels, loc="center left", bbox_to_anchor=(-0.7, 0.5), fontsize=7.5,
    ...
    def _draw_bar_chart(self, cur):
        ...
        if total_days <= 60:
            ...
            for t in self._all_txs:
                d = datetime.strptime(t["transaction_date"], "%Y-%m-%d").date()
                if d in buckets:
                    val = convert_amount(t["amount"], t.get("currency", "INR"), cur)
        ...
        max_val = max(max(income_vals + expense_vals + [1]), 1)
        ax.set_ylim(0, max_val * 1.25)

        ax.bar(x, income_vals, width=0.4, label="Income", color="#22c55e", alpha=0.9, edgecolor="#16a34a", linewidth=0.5)
        ax.bar([i + 0.4 for i in x], expense_vals, width=0.4, label="Expenses", color="#ef4444", alpha=0.9, edgecolor="#b91c1c", linewidth=0.5)

        ax.set_xticks([i + 0.4 for i in x])
        tick_step = max(1, len(labels) // 8)
        vis = [l if i % tick_step == 0 else "" for i, l in enumerate(labels)]
        ax.set_xticklabels(vis, fontsize=8, color="#cbd5e1", rotation=30, ha="right")

        ax.yaxis.set_visible(True)
        ax.tick_params(axis="y", colors="#64748b", labelsize=8)
        ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda v, _: f"{v:,.0f}"))
        ax.grid(axis="y", color="#1e2140", linewidth=0.6, alpha=0.8)
        ax.set_axisbelow(True)

        leg = ax.legend(fontsize=8, facecolor="#1a1f3a", edgecolor="#1e2140", labelcolor="#e2e8f0",
                        loc="upper left", framealpha=0.95)
        leg.get_frame().set_linewidth(0.5)

        for spine in ax.spines.values():
            spine.set_color("#1e2140")
    ...
    def _delete_transaction(self, tx):
        ok = messagebox.askyesno("Delete Transaction",
                                 f"Delete this {tx['type']} of {tx.get('currency', 'INR')}{tx['amount']:,.2f}?\n"
                                 f'"{tx.get("description") or tx["category_name"]}"',
                                 parent=self.winfo_toplevel())
        if ok:
            try:
                self.api.delete_transaction(tx["id"], soft=False)
                self._refresh_data()
            except Exception as e:
                messagebox.showerror("Error", str(e), parent=self.winfo_toplevel())

    def _edit_transaction(self, tx):
        EditTransactionPopup(self.winfo_toplevel(), self.api, self.user, tx,
    ...
    def _refresh_tx_list(self, cur):
        ...
        for tx in txs:
            ...
            tx_date = datetime.strptime(tx["transaction_date"], "%Y-%m-%d").date()
            converted = convert_amount(tx["amount"], tx.get("currency", "INR"), cur)
            sym = SYMBOLS.get(cur, "")

            ctk.CTkLabel(row, text=tx_date.strftime("%d %b %Y"),
                        font=ctk.CTkFont(size=12), width=100, anchor="w",
                        text_color=COLORS["text_secondary"]).pack(side="left")
            ctk.CTkLabel(row, text=tx["category_name"],
                        font=ctk.CTkFont(size=12), width=100, anchor="w",
                        text_color=COLORS["text_secondary"]).pack(side="left")
            ctk.CTkLabel(row, text=tx.get("description") or "",
                        font=ctk.CTkFont(size=12), width=100, anchor="w",
                        text_color=COLORS["text_secondary"]).pack(side="left")
...
class EditTransactionPopup(ctk.CTkToplevel):
    ...
    def __init__(self, master, api, user, tx, on_save):
    ...
    def _build_form(self):
        ...
        COLORS_L = COLORS
        row = 0
        ctk.CTkLabel(main, text="Edit Transaction", font=ctk.CTkFont(size=20, weight="bold"),
                     text_color=COLORS_L["text_primary"]).grid(row=row, column=0, columnspan=2, pady=(0, 16))
        row += 1

        ctk.CTkLabel(main, text="Type", font=ctk.CTkFont(size=12),
                     text_color=COLORS_L["text_secondary"]).grid(row=row, column=0, sticky="w", padx=(0, 12), pady=6)
        self._type_var = ctk.StringVar(value=self.tx["type"].title())
        seg = ctk.CTkSegmentedButton(main, values=["Expense", "Income"],
                                     variable=self._type_var, selected_color=COLORS_L["accent"],
                                     font=ctk.CTkFont(size=12, weight="bold"),
        ...
        self._cat_menu = ctk.CTkOptionMenu(main, variable=self._cat_var, values=["Loading..."],
                                           fg_color="#1a1f3a", button_color=COLORS_L["accent"],
        ...
        self._amount_entry = ctk.CTkEntry(main, height=38, corner_radius=8,
                                          border_color=COLORS_L["border"], font=ctk.CTkFont(size=14))
        ...
        ctk.CTkLabel(main, text="Currency", font=ctk.CTkFont(size=12),
                     text_color=COLORS_L["text_secondary"]).grid(row=row, column=0, sticky="w", padx=(0, 12), pady=6)
        self._cur_var = ctk.StringVar(value=self.tx.get("currency", "INR"))
        ...
        self._date_entry.insert(0, self.tx["transaction_date"])
        row += 1
    ...
    def _load_categories(self):
        ...
        try:
            ...
            self._cat_menu.configure(values=["Error loading"], state="disabled")
